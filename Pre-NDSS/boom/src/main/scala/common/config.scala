...
class DefaultBoomConfig extends Config((site, here, up) => {
   ...
   case BoomTilesKey => up(BoomTilesKey, site) map { r => r.copy(
      core = r.core.copy(
         ...
         numLsuEntries = 16,
         maxBrCount = 8,
         btb = BTBsaParameters(nSets=256, nWays=4, nRAS=16, tagSz=20),
         enableBranchPredictor = true,
         tage = Some(TageParameters()),
         nPerfCounters = 29,
         fpu = Some(freechips.rocketchip.tile.FPUParams(sfmaLatency=4, dfmaLatency=4, divSqrt=true))),
      btb = Some(BTBParams(nEntries = 0, updatesOutOfOrder = true)),
      dcache = Some(DCacheParams(rowBits = site(SystemBusKey).beatBits, nSets=64, nWays=16, nMSHRs=4, nTLBEntries=32)),
      icache = Some(ICacheParams(fetchBytes = 4*4, rowBits = site(SystemBusKey).beatBits, nSets=128, nWays=4, nTLBEntries=16))
      )}
