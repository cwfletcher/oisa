...
object AluRRdDecode extends RRdDecodeConstants
{
   val table: Array[(BitPat, List[BitPat])] =
         ...
         BitPat(uopCMOV)  -> List(BR_N , Y, N, N, FN_CMOV  , DW_XPR, OP1_RS1 , OP2_RS2 , IS_X, REN_1, CSR.N),
         ...
         BitPat(uopCMOVW) -> List(BR_N , Y, N, N, FN_CMOV  , DW_32 , OP1_RS1 , OP2_RS2 , IS_X, REN_1, CSR.N),

         BitPat(uopSEAL)  -> List(BR_N , Y, N, N, FN_SEAL  , DW_XPR, OP1_RS1 , OP2_IMM , IS_I, REN_1, CSR.N),
         BitPat(uopUNSEAL)-> List(BR_N , Y, N, N, FN_UNSEAL, DW_XPR, OP1_RS1 , OP2_IMM , IS_I, REN_1, CSR.N)
       )
...
class RegisterReadDecode(supported_units: SupportedFuncUnits)(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   require (rrd_cs.op_fcn.getWidth == SZ_ALU_FN)
