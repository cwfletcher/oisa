...
{
   val prs1              = UInt(width = preg_sz)  // the register mapped to RS1
   val prs2              = UInt(width = preg_sz)  // the register mapped to RS2
   val prs3              = UInt(width = preg_sz)  // the register mapped to RS3(only for FMA instructions)
   val stale_pdst        = UInt(width = preg_sz)  // the register mapped to the old RD(we will put it into freelist as soon as the new RD is commit) 
