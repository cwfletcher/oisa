...
class MulDiv(cfg: MulDivParams, width: Int, nXpr: Int = 32) extends Module {
  ...
  //printf("req.valid = %d, req.in1 = %x, req.in2 = %x\n", io.req.valid, io.req.bits.in1, io.req.bits.in2)
  //printf("divEarlyOut = %c\n", Mux(cfg.divEarlyOut, Str("Y"), Str("N")))
  //printf("w = %d, mulw = %d\n", w, mulw)
  //printf("Before MulDiv updates: count = %x\n", count)
  //printf("current state: state = %x\n\n", state)
  ...
  if (cfg.divUnroll != 0) when (state === s_div) {
    ...
    // when req.rd_tag == 1, we disable the realy out function
    if (cfg.divEarlyOut) {
      ...
      when (eOut) {
        ...
        remainder := Mux(req.rd_tag, remainder, remainder(w-1,0) << shift)
        count := Mux(req.rd_tag, count, inc)
      }
  //printf("divisor = %x, remainder = %x\n", divisor, remainder)
  //printf("divisorMSB = %x, dividendMSB = %x\n", divisorMSB, dividendMSB)
  //printf("eOutPos = %x, eOutZero = %d, eOut = %d\n", eOutPos, eOutZero, eOut)
    }
  ...
  //printf("After MulDiv updates: count = %x\n", count)
  //printf("current state: state = %x\n\n", state)
  ...
  io.resp.bits.rd_tag := req.rd_tag
