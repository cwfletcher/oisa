...
class NonBlockingDCacheModule(outer: NonBlockingDCache) extends HellaCacheModule(outer) {
  ...
  dtlb.io.req.bits.passthrough := s1_req.phys // If memory unit gives physical address, we just bypass the tlb lookup here
  ...
  val s1_addr = dtlb.io.resp.paddr  // physical register read from DTLB(if resolved in LSU, passthrought == true)
  ...
  val s1_tag_eq_way = wayMap((w: Int) => meta.io.resp(w).tag === (s1_addr >> untagBits)).asUInt   // only check if tag value matches
  val s1_tag_match_way = wayMap((w: Int) => s1_tag_eq_way(w) && meta.io.resp(w).coh.isValid()).asUInt // check other metadata like coherence, dirty
