...
object ALU
{
  ...
  def FN_SLTU = UInt(14)
  def FN_SGEU = UInt(15)
  def FN_CMOV = UInt(16)  // new instruction
  def FN_SEAL = UInt(17)
  def FN_UNSEAL = UInt(18)
...
class ALU(implicit p: Parameters) extends CoreModule()(p) {
  val io = new Bundle {
    ...
    val in0 = UInt(INPUT, xLen) // only for cmov
  ...
  // CMOV, SEAL, UNSEAL
  val cmov_out = Mux(io.in1 === UInt(0), io.in0, io.in2)
  val final_out = Mux(io.fn === FN_SEAL || io.fn === FN_UNSEAL, io.in1, Mux(io.fn === FN_CMOV, cmov_out, out))
  io.out := final_out
