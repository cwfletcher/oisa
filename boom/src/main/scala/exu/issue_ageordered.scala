   ...
   extends IssueUnit(params.numEntries, params.issueWidth, num_wakeup_ports, params.iqType)
{
   ...
   val MAX_SHIFT = DISPATCH_WIDTH   // = decodeWidth
   ...
   for (w <- 0 until issue_width)
   {
      ...
      io.iss_uops(w).pdst := 0.U
      io.iss_uops(w).lrs1_rtype := RT_X
      io.iss_uops(w).lrs2_rtype := RT_X
      io.iss_uops(w).dst_rtype  := RT_X
   }

   val requests = issue_slots.map(s => s.request)   // all issue slots ready to issue
