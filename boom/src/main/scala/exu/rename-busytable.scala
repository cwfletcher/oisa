   ...
   (implicit p: Parameters) extends BoomModule()(p)
{
   ...
   // namely, if instr2's RS1/RS2/RD === instr1's RD, then instr2's RS1/RS2/RD's physical register is not readed from maptable
   // instead, it comes from the freelist as instr1' RD, and will definitely be busy
   // To be more clear, even if the map table + busy table shows that preg mapped to instr2's RD is not busy. Since
   // instr2's RD is actually instr1's RD, it is definitely busy.
   ...
   val pdst_was_bypassed = Wire(init = Vec.fill(pl_width) {Bool(false)})
   for {
      w <- 0 until pl_width
      xx <- w-1 to 0 by -1
   }{
      ...
      when (io.ren_uops(w).dst_rtype  === UInt(rtype) && io.ren_will_fire(xx) && io.ren_uops(xx).ldst_val && io.ren_uops(xx).dst_rtype === UInt(rtype) && (io.ren_uops(w).ldst === io.ren_uops(xx).ldst))
         { pdst_was_bypassed(w) := Bool(true) }
   ...
   for (w <- 0 until pl_width)
   {
      ...
      if (rtype == RT_FLT.litValue)
      {
         ...
         io.values(w).pdst_busy := Bool(false)
      }
      else
      {
         busy_table.io.prs(2,w) := io.map_table(w).stale_pdst
         io.values(w).prs3_busy := Bool(false)
         io.values(w).pdst_busy := io.ren_uops(w).dst_rtype === UInt(rtype) && (busy_table.io.prs_busy(2,w) || pdst_was_bypassed(w))
      }
