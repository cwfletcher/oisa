...
//         ("fp interlock", () => id_ex_hazard && ex_ctrl.fp || id_mem_hazard && mem_ctrl.fp || id_wb_hazard && wb_ctrl.fp || id_ctrl.fp && id_stall_fpu)))),
   ...
   for ((renport, intport) <- rename_stage.io.int_wakeups zip int_wakeups)
   {
      ...
      renport.valid :=
         intport.valid &&
         !(sxt_ldMiss && (intport.bits.uop.iw_p1_poisoned || intport.bits.uop.iw_p2_poisoned || intport.bits.uop.iw_pd_poisoned))
   ...
   val iss_loadIssued =
      ...
      !(sxt_ldMiss && (mem_iq.io.iss_uops(0).iw_p1_poisoned || mem_iq.io.iss_uops(0).iw_p2_poisoned || mem_iq.io.iss_uops(0).iw_pd_poisoned))
   sxt_ldMiss :=
   ...
   for {
      ...
      issport.bits.poisoned := wakeup.bits.uop.iw_p1_poisoned || wakeup.bits.uop.iw_p2_poisoned || wakeup.bits.uop.iw_pd_poisoned
   ...
   for (w <- 0 until exe_units.length)
   {
      iregister_read.io.iss_valids(w) :=
         iss_valids(w) && !(sxt_ldMiss && (iss_uops(w).iw_p1_poisoned || iss_uops(w).iw_p2_poisoned || iss_uops(w).iw_pd_poisoned))
   }
   iregister_read.io.iss_uops := iss_uops
   iregister_read.io.iss_uops map { u => u.iw_p1_poisoned := false.B; u.iw_p2_poisoned := false.B; u.iw_pd_poisoned := false.B; }
   ...
   for (i <- 0 until exe_units.length)
   {
      for (j <- 0 until exe_units(i).num_rf_write_ports)
      {
         ...
         if (exe_units(i).uses_csr_wport && (j == 0))
         {
            ...
            iregfile.io.write_ports(w_cnt).bits.ift_tag := wbrd_tag
