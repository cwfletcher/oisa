...
class ALUExeUnit(
   ...
   extends ExecutionUnit(
      num_rf_read_ports = 3, //if (has_fpu) 3 else 2,
      ...
      has_ifpu = has_ifpu)(p)
{
   ...
   // ALU Unit -------------------------------
   var alu: ALUUnit = null  // IFT propagation inside ALUUnit(uop out has right rd_tag)
   if (has_alu)
   {
      ...
      alu.io.req.bits.rd_data  := io.req.bits.rd_data     // useful only for CMOV
   ...
   // Pipelined, IMul Unit ------------------
   var imul: PipelinedMulUnit = null  // IFT propagation inside PipelinedMulUnit(uop out has right rd_tag)
   ...
   // FPU Unit -----------------------
   var fpu: FPUUnit = null  // IFT propagation insided FPUUnit(uop out has right rd_tag)
   ...
   // FDiv/FSqrt Unit -----------------------
   var fdivsqrt: FDivSqrtUnit = null  // IFT is doing below(set fdiv_resp_uop.rd_tag = fdivsqt.io.resp.rs1_tag || rs2_Tag
   ...
   if (has_fdiv)
   {
      ...
      fdiv_resp_uop.rd_tag := (fdivsqrt.io.resp.bits.uop.rs1_tag && fdivsqrt.io.resp.bits.uop.lrs1_rtype != RT_X) ||
                              (fdivsqrt.io.resp.bits.uop.rs2_tag && fdivsqrt.io.resp.bits.uop.lrs2_rtype != RT_X)
   ...
   // Mul/Div/Rem Unit -----------------------
   var muldiv: MulDivUnit = null    // IFT propagation is inside MulDivUnit (uop out has right rd_tag)
      ...
      has_fpiu = has_fpiu)(p)
{
   ...
   // FPU Unit -----------------------
   var fpu: FPUUnit = null  // IFT propagation is inside FPUUnit (uop out has right rd_tag)
   val fpu_resp_val = Wire(init=Bool(false))
   val fpu_resp_fflags = Wire(new ValidIO(new FFlagsResp()))
   fpu_resp_fflags.valid := Bool(false)
   if (has_fpu)
   ...
   // FDiv/FSqrt Unit -----------------------
   var fdivsqrt: FDivSqrtUnit = null  // IFT propagation is below (fdiv_resp_uop.rd_tag = fdivsqrt.io.resp.rs1_tag || rs2_tag)
   val fdiv_resp_val = Wire(init=Bool(false))
   val fdiv_resp_uop = Wire(new MicroOp())
   val fdiv_resp_data = Wire(Bits(width=65))
   val fdiv_resp_fflags = Wire(new ValidIO(new FFlagsResp()))
...
class FDivSqrtExeUnit(implicit p: Parameters) // Never used
   ...
   uses_iss_unit = false)
{
   ...
   val ifpu = Module(new IntToFPUnit(latency=intToFpLatency)) // IFT propagation is inside IntToFPUnit
   ifpu.io.req <> io.req
