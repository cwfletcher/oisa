...
{
   // inputs from dispatch stage(renaming logic)
   ...
   // output as issuing to execute stage
   ...
   with IssueUnitConstants
{
   ...
   for (w <- 0 until DISPATCH_WIDTH)
   {
      ...
      dis_uops(w).iw_pd_poisoned := false.B
   ...
   if (DEBUG_PRINTF)
   {
      ...
      {
         printf("  " + this.getType + "_issue_slot[%d](%c)(Req:%c):wen=%c P:(%c,%c,%c,%c) OP:(%d,%d,%d) PDST:%d Stale_PDST:%d %c [[DASM(%x)]" +
