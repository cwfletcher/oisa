...
trait IFTTagType
{
  val public = Bool(false)
  val confidential = Bool(true)
}
...
//   val uopSRET = UInt(40, UOPC_SZ)
   ...
   val uopCMOV      = UInt(124, UOPC_SZ)
   val uopCMOVW     = UInt(125, UOPC_SZ)

   val uopSEAL      = UInt(126, UOPC_SZ)
   val uopUNSEAL    = UInt(127, UOPC_SZ)
