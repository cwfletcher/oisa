   ...
   with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
   ...
   // IFT tag for all operands
   val rs1_tag          = Bool()
   val rs2_tag          = Bool()
   val rs3_tag          = Bool()  // only useful in FMA etc. Must be public in other instructions
   val rd_tag           = Bool()  // generated by the functional unit

   // What is the next state of this uop in the issue window? useful for the compacting queue.
   ...
   // so we can ignore p3.
   val iw_pd_poisoned   = Bool()    // for cmov, dst register can be waked up speculatively
   ...
   val pdst             = UInt(width = PREG_SZ)       // new physical register mapped to RD(come from freelist)
   val pop1             = UInt(width = PREG_SZ)       // physical reg mapped to RS1 right now
   val pop2             = UInt(width = PREG_SZ)       // physical reg mapped to RS2 right now
   val pop3             = UInt(width = PREG_SZ)       // physical reg mapped to RS3 right now(only used for FMA etc.)
   val stale_pdst       = UInt(width = PREG_SZ)       // physical reg mapped to RD right now

   val prs1_busy        = Bool()
   val prs2_busy        = Bool()
   val prs3_busy        = Bool()
   val pdst_busy        = Bool()  // if the physical mapped to RD(old one) is busy(the new one is of course busy)
   val exception        = Bool()
   ...
   val dst_rtype        = UInt(width=2)       // notice that dst_rtype == stale_dst_rtype(same logic register, different physical register)
